...
// This is the primary entry point for creating an Instana API
//
// Parameters:
...
//
// Returns:
//   - InstanaAPI: The API client interface for accessing all Instana resources
//
// Example:
//
...
func NewInstanaAPI(apiToken string, endpoint string, skipTlsVerification bool) InstanaAPI {
	...
	return NewInstanaRestAPI(restClient)
}
...
//	    log.Fatal(err)
//	}
func NewInstanaAPIWithConfig(cfg *config.ClientConfig) (InstanaAPI, error) {
	restClient, err := NewClientWithConfig(cfg)
	if err != nil {
		return nil, err
	}

	return NewInstanaRestAPI(restClient), nil
}
