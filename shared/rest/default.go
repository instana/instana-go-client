...
func (r *defaultRestResource[T]) upsertWithNoContentHandling(data T, operation restClientOperation) (T, error) {
	...
	// Handle 204 No Content response or empty response
	// This happens when API returns empty JSON object "{}" or completely empty response
	responseStr := string(response)
	if responseStr == "{}" || responseStr == "" || len(response) == 0 {
		return data, nil
	}
	return r.validateResponseAndConvertToStruct(response)
}
